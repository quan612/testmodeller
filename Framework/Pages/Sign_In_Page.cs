...
namespace Pages
{
    public class Sign_In_Page : BasePage
    {
        public Sign_In_Page (IWebDriver driver)
            : base(driver)
        {
	...
	private By Password_is_requiredElem = By.XPath("//SPAN[contains(text(),'Password is required.')]");
 	...
 	public void Click_Username_is_required()
 	{
 		IWebElement elem = getWebElement(Username_is_requiredElem);

 		if (elem == null) {
     		TestModellerLogger.FailStepWithScreenshot(m_Driver, "Click_Username_is_required", "Click_Username_is_required failed. Unable to locate object: " + Username_is_requiredElem.ToString());
		...
		TestModellerLogger.PassStep(m_Driver, "Click_Username_is_required");
 	}
     
 	public void Click_Password_is_required()
 	{
